...
public class MongoAccess {
    ...
    public List<Document> findTweetsByScreenName(List<String> screenNames) {
        MongoCollection<Document> tweetsAdded = mongoConnection.getMongoDatabase().getCollection("tweets_added");
        BasicDBObject getQuery = new BasicDBObject();
        for (String name : screenNames) {
            getQuery.put("user.screen_name", Pattern.compile(name, Pattern.CASE_INSENSITIVE));
        }
        ArrayList<Document> allTweets = tweetsAdded.find(getQuery).into(new ArrayList<Document>());
        log.info("All tweets from this screen name have been added to list: {}", allTweets.toString());
